...
function placeTerrainSegment(leftPoint, rightPoint) {
  let leftVector = new THREE.Vector2(
    leftPoint[0] - halfWidth,
    leftPoint[1] - halfHeight
  );
  let rightVector = new THREE.Vector2(
    rightPoint[0] - halfWidth,
    rightPoint[1] - halfHeight
  );
...
function handleMovement() {
  ...
  if (accelerating && currentFuel > 0) {
    ...
    if (currentAcceleration > thrusterAccelerationMax)
      currentAcceleration = thrusterAccelerationMax;

    currentAccelerationRatio = currentAcceleration / thrusterAccelerationMax;
    let fuelBurned =
      currentAccelerationRatio *
        (fuelConsumptionRateMax - fuelConsumptionRateMin) +
      fuelConsumptionRateMin;
    ...
    currentAcceleration -=
      thrusterJerk * deltaTime * accelerationFalloffMultiplier;
...
function borderCheck() {
  if (
    lander.position.x < -halfWidth - lineWidth ||
    lander.position.x > halfWidth + lineWidth ||
    lander.position.y < -halfHeight - lineWidth ||
    lander.position.y > halfHeight + lineWidth
  ) {
...
function terrainCheck() {
  ...
  for (i = 0; i < spheres.length; i++) {
    ...
    for (j = 0; j < lineSegments.length; j++) {
      ...
      if (
        i == 0 &&
        worldPosition.x > leftVector.x &&
        worldPosition.x < rightVector.x
      ) {
        const k =
          (rightVector.y - leftVector.y) / (rightVector.x - leftVector.x);
        const terrainHeight =
          leftVector.y + k * (worldPosition.x - leftVector.x);
        altitude = worldPosition.y - mainScale - smallScale - terrainHeight;
        altitude = Math.max(0, altitude);
      }

      let dot =
        ((worldPosition.x - leftVector.x) * (rightVector.x - leftVector.x) +
          (worldPosition.y - leftVector.y) * (rightVector.y - leftVector.y)) /
        leftVector.distanceToSquared(rightVector);
      ...
      if (
        closestX < leftVector.x ||
        closestX > rightVector.x ||
        closestY > Math.max(leftVector.y, rightVector.y) ||
        closestY < Math.min(leftVector.y, rightVector.y)
      )
        continue;
      ...
      if (distanceSquared <= hitDistance * hitDistance) {
        ...
        if (
          Math.min(
            lander.position.x - leftVector.x,
            rightVector.x - lander.position.x
          ) <
          landerScale * 0.38
        ) {
        ...
        if (
          ...
          landingVelocityTolerance * landingVelocityTolerance
        ) {
...
function crashed() {
  ...
  fuelChange =
    Math.random() * (fuelPenaltyMax - fuelPenaltyMin) + fuelPenaltyMin;
...
function randomSpawn() {
  ...
  let initialVelocity =
    Math.random() * (velocityXSpawnMax - velocityXSpawnMin) + velocityXSpawnMin;
...
function cameraPositionReset() {
  camera.position.set(
    cameraPositionDefault.x,
    cameraPositionDefault.y,
    cameraPositionDefault.z
  );
}

function checkZoom() {
  if (
    altitude <= cameraZoomAltitude ||
    (isZoomed && altitude <= cameraZoomOutAltitude)
  ) {
    zoomIn();
